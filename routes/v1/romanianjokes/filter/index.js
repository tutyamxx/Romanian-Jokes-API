...
router.get("/:joketype", async (req, res, next) =>
{
    ...
    if (!getJokeType.length) return res.status(404).json({ message: "This joke type could not be found in the jokes list" });
...
router.get("/:joketype/random", async (req, res, next) =>
{
    ...
    const getJokeType = await mongoGetRandom(dbName, dbCollection, pipeLine);
